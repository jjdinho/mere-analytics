...
**Shipped (engineering-review deltas):** The build above is the pre-review plan; the live implementation refined it. The historical record is preserved verbatim; what actually landed:
- **Package layout.** One flat `internal/mcp` package (`mcp.go` server/transport wiring, `adapter.go` panic-recovery, `tools.go` query+schema tools) instead of a `tools/` subpackage — a subpackage would import the parent for `registerTool` and create a cycle. The `/mcp` route mounts in `internal/web/server.go` alongside `/api/v1/*` (the repo has no `internal/web/handlers/` dir; handlers are flat files), not in a separate `handlers/mcp.go`. The upstream `github.com/mark3labs/mcp-go/mcp` package is imported as `mcpgo` to avoid colliding with our own package name.
- **Project comes from the bearer grant, not a tool argument.** The `/mcp` endpoint is single, path-less, so a tool can't take a `:project_id`. The OAuth access token is already bound to one (user, project) at consent, so the tool handlers read `oauth.FromContext(ctx).ProjectID` (the request context with the `RequireBearer`-attached `AccessContext` flows through the stateless Streamable HTTP transport into the handler). The query/schema tools take no project argument.
- **Visibility re-check for soft-delete parity.** Each tool calls the auth viewer (`auth.NewViewer(...).Projects(ctx).ByID(...)`) before touching ClickHouse, so a project soft-deleted (or membership revoked) within the token's 1h life denies with a `project not found` tool-error — mirroring decision #11's 404 on the HTTP API. This is why `mcp.Deps` carries the `*auth.Service` alongside the shared executor + schema provider.
- **Errors are tool-errors, not protocol errors.** A ClickHouse parse/timeout/memory error, empty/missing `sql`, or breaching the 1000-row cap come back as `CallToolResult{IsError:true}` (verbatim CH message, same as the HTTP API) so the model reads and self-corrects. Only a panic or an infrastructure failure (e.g. Postgres down in the visibility check) becomes a JSON-RPC `internal_error` (-32603).
- **Transport.** `server.NewStreamableHTTPServer(..., WithStateLess(true))` — the bearer token carries identity, so there's no session store to keep. CORS + `RequireBearer` wrap it exactly as `/api/v1/*`; `/mcp` was already CSRF-exempt in `authMiddleware`.
- **`mark3labs/mcp-go` pinned to `v0.54.1`, a tagged release, not a bare commit SHA.** Go records the version in `go.mod` and its content hash in `go.sum`, which makes a tag as immutable + reproducible as a SHA (the proxy + checksum DB reject a moved tag). `go get module@<sha>` resolves back to the tag anyway when the commit is tagged, so a pseudo-version would require deliberately picking an *un*released commit — strictly worse. The reproducibility intent of the original "pin to a SHA" decision is satisfied by the tag + go.sum.
...
**Shipped (engineering-review deltas):** The config artifacts above are the pre-review plan; the live implementation refined them. The historical record is preserved verbatim; what actually landed:

- **`mere_readonly` source-of-truth — option (A).** `config/deploy/clickhouse/users.xml` defines only `default` (loopback-only) and `mere_admin`; `mere_readonly` stays app-provisioned on every boot (`ProvisionReadonlyUser`). Single source of truth, no botched-rotation divergence.
- **CH admin password via `from_env`, not an envsubst hook.** `users.xml` reads the sha256 digest with `<password_sha256_hex from_env="CLICKHOUSE_ADMIN_PASSWORD_SHA256">`. ClickHouse substitutes it at startup, so there is no pre-deploy rendering step and no `.tpl` file — strictly simpler than the "small envsubst-style pre-deploy hook" the Deploy section sketched. The plaintext still never reaches the CH server. With option (A) the readonly hash is unused server-side (the app provisions readonly from the plaintext `CLICKHOUSE_READONLY_PASSWORD`), so only the admin digest is passed to the accessory.
- **`config/deploy.yml` is gitignored; only `config/deploy.example.yml` is committed.** Resolves the plan's internal contradiction (repo-layout said "committed", but the operator workflow does `cp deploy.example.yml deploy.yml`). The example is the template; the operator's filled-in copy is local. Standard Kamal-OSS shape.
- **`OAUTH_ISSUER_URL` added to the app env; `SESSION_SECRET` dropped.** The draft `deploy.yml` env predates the OAuth server and PG-backed sessions: `OAUTH_ISSUER_URL` is `config`-required (the app won't boot without it) and was missing; `SESSION_SECRET` is referenced nowhere in Go (sessions are PG-backed) and was cargo-culted.
- **Operator scripts are mounted into the accessories.** The draft aliases ran `-f /operator/*.sql` but nothing put them there. The example now mounts `create-user.sql` + `reset-password.sql` into the postgres accessory and `wipe-project.sql` into the clickhouse accessory via `files:`, so the aliases actually resolve.
- **`wipe-project` ships as a `.sql` file, not inline.** `scripts/operator/wipe-project.sql` runs `ALTER TABLE analytics.events_raw_v1 DELETE WHERE project_id = {project_id:UUID}`, invoked with `clickhouse-client --param_project_id=$PROJECT_ID --queries-file ...`. Typed param binding (UUID, matching the column) avoids the nested-quote/injection surface of the inline `--query "...'$PROJECT_ID'..."` form and is consistent with the file-based create-user/reset-password scripts.
- **`create-user` kept in the alias set.** This step's "operator alias surface" line predates the post-step-4 follow-up that added `create-user`; decision #15 is authoritative. Shipped aliases: `create-user`, `reset-password`, `wipe-project`, `db-console`, `clickhouse-console`, plus Kamal-default `console` + `logs`.
- **`registry.server: ghcr.io` set explicitly.** Required to actually push to GHCR (the draft omitted it). Self-hosters override or drop it for Docker Hub.
- **`drain_timeout: 30` is a root-level key, not under `proxy:`.** In Kamal 2 the proxy block has no `drain_timeout`; it's a top-level deploy key (whose default is already 30s). Set explicitly so the must-exceed-SIGTERM-budget constraint is visible. `builder.arch: amd64` + `builder.args.VERSION` are the correct Kamal 2 keys.
- **Build-time version stamp landed.** `cmd/server` has `var Version = "dev"`, injected via the Dockerfile's `ARG VERSION` → `-ldflags="-X main.Version=…"` (`deploy.example.yml`'s `builder.args.VERSION` evaluates `git describe --tags --always --dirty` at deploy time). It is logged on boot (`starting mere-server version=…`) and returned in the `/healthz` JSON body (`"version"`). Covered by `internal/web` unit test + the e2e boot test (`"version":"dev"` for the un-stamped build).
- **`config.xml`** ships the async-insert `default` profile plus 30-day TTLs on the noisy system-log tables (`query_log`, `query_thread_log`, `part_log`, `trace_log`, `metric_log`, `asynchronous_metric_log`); `<listen_host>` is intentionally left to the image's `docker_related_config.xml`.
